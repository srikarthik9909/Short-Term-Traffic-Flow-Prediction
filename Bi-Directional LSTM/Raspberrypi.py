...
time_steps = 15
BI_LSTM_X, y = create_sequences(column_data, time_steps)
print(f"Shape of LSTM_X: {BI_LSTM_X.shape}")
...
for i in range(len(BI_LSTM_X)):
    cnn_input_data = np.expand_dims(BI_LSTM_X[i], axis=0).astype(np.float32)
